...
def preprocess_process(data: pd.DataFrame, params: dict) -> pd.DataFrame:
    ...
    ohe = load_joblib(path = "data/processed/ohe_home_ownership.pkl")
    
    cat_ohe_data = preprocess_ohe(data = cat_data, ohe = ohe, column = "person_home_ownership")
    
    ohe = load_joblib(path = "data/processed/ohe_loan_intent.pkl")
    
    cat_ohe_data = preprocess_ohe(data = cat_ohe_data, ohe = ohe, column = "loan_intent")

    ohe = load_joblib(path = "data/processed/ohe_loan_grade.pkl")
    
    cat_ohe_data = preprocess_ohe(data = cat_ohe_data, ohe = ohe, column = "loan_grade")
